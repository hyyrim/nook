...
| 항목 | 상태 |
|------|------|
| `landing/` 독립 단일 페이지 — 앱 빌드·Expo Router 구조와 분리 (→ 결정 112) | ✅ |
| 실제 앱 아이콘과 0726 스크린샷 기반 반응형 제품 소개 | ✅ |
| App Store 다운로드·Google Form 버그 제보·지원 이메일 연결 | ✅ |
| 기존 개인정보 처리방침·서비스 이용약관 연결 | ✅ |
| 공유용 Open Graph 이미지와 메타데이터 적용 | ✅ |
| 피드백 반영 — hero 빨간 점 제거·헤더 로고 교체·화면 섹션 제목 한 줄 처리 | ✅ |
| 모바일 피드백 반영 — 설명 줄바꿈·발견 콘텐츠 점 제거·앱 화면 가로 스크롤/스냅 | ✅ |
| 모바일 앱 화면 목록의 추가 제스처 강제·스와이프 힌트는 피드백에 따라 복원 | ✅ |
| Sites 프로덕션 배포 및 공개 접근 전환 | ✅ |
| Vercel 호환 빌드 및 `nook-landing-page.vercel.app` 공개 배포 | ✅ |
| 앱 화면 목록 내부 세로 overflow 차단 — 가로 스크롤·페이지 세로 스크롤·하단 캡션 노출 유지 | ✅ |
