...
최종 업데이트: 2026-06-17 (10차 — 폴더 상세 다중 편집)
...
| 카테고리 추가 후 자동 스크롤 + 하이라이트 | 폴더 탭에서 새 카테고리 생성 시, 해당 카드 위치까지 smooth scroll + 잠깐 테두리/배경 하이라이트로 추가 확인 (현재는 created_at asc 정렬이라 하단에 생성되어 가려짐) |
