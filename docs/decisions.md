...
## 109. TestFlight 발견 이슈 P0-P3 안정화 배치 (2026-07-23)

**결정**: TestFlight 실사용 중 발견된 계정 격리 잔상, 계정 삭제 보호, 온보딩 권한 안내, 카테고리 바텀시트 키보드 회피, 리마인더 삭제 UX, 주요 빈 상태 중앙 배치를 한 번의 안정화 배치로 처리한다.

**배경**: 새 계정 로그인 직후 홈에서 이전 계정 콘텐츠가 새로고침 전까지 보이는 현상은 실제 쿼리의 `user_id` 스코프보다 화면 로컬 상태와 늦게 도착한 이전 요청 결과가 더 큰 원인이었다. 계정 삭제는 기존 Alert가 있었지만 TestFlight에서 확인 절차가 충분히 분명하지 않게 느껴졌고, 온보딩/폴더 카테고리 추가 시트는 키보드와 높이 제한이 충돌했다. 리마인더 목록은 개별 삭제 후 전체 로딩으로 화면이 깜빡였고, Report/Search/Folder Detail의 empty state는 예정된 리마인더 화면보다 시각 중심이 위로 치우쳤다.

**결과**:
- 홈은 `session.user.id` 변경 시 화면 상태를 즉시 비우고, 이전 userId로 시작한 비동기 load 결과는 state에 반영하지 않는다.
- Root Stack에 user-scoped key를 부여해 로그인 사용자 변경 시 탭/상세 화면 로컬 상태가 재사용되지 않게 했다.
- 계정 삭제는 1차 Alert 뒤 마지막 확인 Alert를 추가하고, 삭제 중 중복 탭을 막는다.
- 알림 권한이 이미 허용/거부된 상태여도 온보딩의 알림 안내 화면은 건너뛰지 않고 보여준다.
- `CategoryBottomSheet`는 `showStyleControls=false` 모드를 추가해 온보딩에서는 이름만 입력한다. 키보드 대응은 padding 확장 대신 시트 자체를 위로 이동시키는 방식으로 변경해 높이 막힘을 줄였다.
- 예정된 리마인더는 개별 삭제 후 전체 reload를 하지 않고 해당 row만 제거한다. 헤더에 `전체 삭제` 액션을 추가하고, 개별 삭제 버튼은 닫기 X에서 trash 아이콘으로 변경했다.
- Report/Search/Category Detail의 empty state는 공통 `EmptyState` center variant와 content container center 정렬로 중앙 배치한다.

**대안 검토**:
- 서버/RLS만 재검토: 필수 확인 대상이지만 현재 클라이언트 API는 대부분 `user_id` 필터를 갖고 있었다. TestFlight 증상은 새로고침 후 사라졌으므로 로컬 상태 잔상과 race를 먼저 제거하는 것이 직접 대응이다.
- 온보딩용 별도 바텀시트 신규 작성: UI는 작아지지만 컴포넌트 중복이 생긴다. 기존 `CategoryBottomSheet`에 style controls 토글만 추가해 유지보수 범위를 좁혔다.
- 리마인더 삭제 후 전체 재조회 유지: OS pending queue와 DB content 매핑을 다시 맞출 수 있지만, 사용자가 본 문제인 전체 깜빡임을 계속 만든다. 삭제 대상은 이미 현재 화면 row로 확정되어 있어 optimistic local removal이 적합하다.

**교훈**: 멀티 유저 앱에서 "쿼리가 안전하다"와 "화면이 안전하게 보인다"는 별개다. 인증 전환 시 로컬 state와 진행 중인 요청 결과까지 user scope로 다뤄야 개인정보 잔상이 생기지 않는다. 또한 온보딩에서는 나중에 쓰일 수 있는 설정을 전부 받기보다, 지금 실제로 반영되는 최소 입력만 받는 편이 더 솔직하고 빠르다.

## 110. 리마인더 UX 재정비 + 리마인더 알림 채널 (2026-07-24)

**결정**: 결정 109의 리마인더/키보드 대응을 실사용 피드백에 맞춰 재정비하고, 콘텐츠 리마인더를 알림 설정의 독립 채널로 게이팅한다. (1) 카테고리 시트 키보드 회피 방식 교체, (2) 예정된 리마인더 삭제 UX를 iOS 표준 스와이프+Undo로 재작성, (3) 리마인더 알림 on/off 토글 신설 + 콘텐츠 상세 벨 게이팅.

**배경**: 109에서 카테고리 시트 키보드 대응을 "시트 자체를 위로 이동(translateY)"으로 바꿨으나, 스타일 컨트롤을 포함한 add 시트(화면 82% 높이)에서는 키보드 높이만큼 통째로 올리면 상단 이름 입력칸이 화면 밖으로 잘렸다. 리마인더 삭제도 109에서 개별 trash 버튼 + 확인 Alert + "전체 삭제" 헤더로 정리했지만, 상시 빨간 버튼은 리스트를 시끄럽게 하고 확인 Alert는 마찰이 컸다. 또 콘텐츠 리마인더(로컬 알림)는 알림 설정과 무관하게 항상 예약 가능해, 사용자가 알림을 끈 상태에서도 벨이 동작하는 불일치가 있었다.

**결과**:
- **카테고리 시트 키보드**: translateY 방식을 폐기하고 `SaveBottomSheet`와 동일한 `paddingBottom += keyboard.height` 방식으로 통일. 흰 시트 배경이 키보드 뒤까지 이어지고(어두운 backdrop 틈 제거), 정적 `maxHeight = windowHeight - topGap`로 상단 잘림을 막고 초과분은 아이콘 그리드 `flexShrink`가 흡수한다.
- **예정된 리마인더 삭제**: `ReanimatedSwipeable` 스와이프 → 삭제 버튼 노출 → 탭 삭제. 확인 Alert 대신 하단 Undo 바(4초, 기존 Toast 스타일)로 되돌림(실행취소 시 `scheduleReminder` 재등록). 한 번에 한 행만 열리게 `onSwipeableOpenStartDrag`에서 이전 행을 닫고, 행 제거는 `LinearTransition` spring으로 정착. 상세 복귀 시 전체 스피너 깜빡임 제거(재포커스는 조용히 갱신). "전체 삭제" 헤더 액션은 스와이프로 대체하며 제거(109 대비 롤백).
- **리마인더 알림 채널**: `notification_settings.content_reminder_enabled`(마이그레이션 020, default true) 신설 + 타입 반영. 알림 설정 "알림 종류"에 리마인더 토글 추가(마스터 off면 함께 비활성). 콘텐츠 상세 벨은 `enabled && content_reminder_enabled`가 아니면 안내 Alert + 알림 설정 이동, 맞으면 시트 오픈. 게이트 상태는 focus 시 미리 조회해 탭 지연이 없다. 기존 예약분은 비파괴적으로 유지(새 예약만 차단, 기존 취소는 사용자가 예정 화면에서 처리).

**대안 검토**:
- **키보드 translateY 유지 + 상단 clamp만 추가**: 우회적. SaveBottomSheet 패턴에 정렬하는 편이 앱 전체 일관성과 "키보드 뒤 흰 배경"까지 동시에 해결.
- **스와이프 즉시 삭제(풀 스와이프)**: 오탭 위험이 커서 reveal-and-tap + Undo로 안전화. 여러 행이 동시에 열리는 문제는 단일 열림으로 해소.
- **리마인더 토글 off 시 기존 예약 전부 취소**: "off=완전 미수신"에 부합하나 비가역 삭제라 기본 비활성. 사용자가 예정 화면에서 직접 취소하도록 위임.
...
## 112. 앱과 독립된 최소 랜딩페이지 구성 (2026-07-31)

**결정**: 앱 런타임과 배포에 영향을 주지 않도록 `landing/`에 독립된 단일 페이지를 두고, 실제 앱 아이콘·스크린샷과 공식 App Store·버그 제보·문의·정책 링크만 사용한다.

**배경**: Nook 앱은 Expo 기반 iOS 제품으로 안정화 중이며, 소개 페이지 때문에 기존 Router 구조나 앱 빌드 설정을 변경할 이유가 없다. 랜딩의 목적은 제품 설명과 다운로드·지원 동선 제공이다.

**결과**:
- `landing/`에 반응형 랜딩페이지와 Sites 배포 설정을 추가했다.
...
**결정**: 웹 저장 시 메타 추출을 `extract-metadata` Supabase Edge Function으로 서버에서 수행하고, 앱은 `lib/metadata.ts`의 `fetchLinkMetadata` 안에서 `Platform.OS==='web'`일 때 이를 invoke한다(iOS는 기존 로직 그대로).

**⚠️ 초기 `metadata.web.ts` 방식 폐기(같은 커밋 내 수정)**: 처음엔 `lib/metadata.web.ts`를 신설해 Metro 플랫폼 해소로 교체하려 했으나, 그 파일 안의 `import { normalizeUrl } from './metadata'`가 **웹에선 `./metadata`를 자기 자신(metadata.web.ts)으로 해소** → `normalizeUrl`이 undefined → `fetchLinkMetadata` 첫 줄에서 throw → 웹 저장 전부 실패. `auth.web.ts`는 자기완결(base를 import 안 함)이라 이 함정을 피했던 것. 교훈: **`.web.ts` 오버라이드는 base 파일을 import/재수출하면 안 된다.** → 파일 삭제하고 `metadata.ts` 내부 `Platform.OS` 분기로 전환(supabase.ts가 쓰는 패턴).
...
**1차 완료 시점의 알려진 한계 (Phase 2 이월, 2026-08-08)**:
- **YouTube 등 봇 차단 사이트의 본문(description)은 웹에서 못 가져온다.** 이유는 코드가 아니라 IP: iOS는 가정용 IP라 YouTube가 정상 페이지(임베디드 JSON 포함)를 주지만, Edge Function은 데이터센터 IP라 봇/consent 페이지를 받아 본문 자체가 응답에 없음. iOS의 본문 추출 로직을 포팅해도 무의미. 필요 시 **YouTube Data API v3**(`videos.list?part=snippet`, 키·일 10k 쿼터·secret 필요)로 해결 — 보조 동선 대비 과해서 1차엔 보류. 태그·카테고리는 제목 기반 classify로 채워져 재분류엔 지장 적음.
- 일반 사이트(블로그·뉴스)는 데이터센터 IP도 정상 페이지를 줘서 제목·본문·썸네일 모두 정상.
