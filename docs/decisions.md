...
## 066. 홈 데이터 상태별 밀도 조절과 dev 케이스 추가 (2026-06-26)

**결정**: 홈에 새 주요 섹션을 추가하지 않고, Rediscover/Forgotten 노출 여부에 따라 최근 저장 노출 개수를 조절한다. 최근 저장으로도 화면 밀도가 부족한 경우에는 작은 보조 카드를 표시한다. 개발 모드에서는 홈 상단에 케이스 칩을 제공해 여러 데이터 상태를 빠르게 확인한다.
...
- Rediscover/Forgotten 섹션에는 별도 `discoverySection` spacing을 적용. Rediscover 다음에 Forgotten이 이어질 때만 28px 간격을 주고, 그 외 discovery 섹션 하단은 24px로 유지
- `__DEV__`에서만 `Live`, `빈 홈`, `최근 적음`, `최근 충분`, `발견만`, `잊음만`, `전체` 케이스를 선택할 수 있는 홈 preview 칩 추가
- dev preview에서는 fake content_id가 analytics나 실제 상세 라우팅에 섞이지 않도록 카드 탭과 Rediscover impression 추적을 비활성화
...
**교훈**: 홈의 빈 느낌은 반드시 새 데이터 섹션으로 해결할 필요가 없다. 섹션 의미는 유지하고, 데이터 상태에 따라 기존 섹션의 밀도를 조절하는 편이 MVP 구조를 더 단단하게 유지한다.
