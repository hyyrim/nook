...
**결정**: 홈 화면에 "잊고 있던 콘텐츠" 섹션을 추가한다. 한 번이라도 열어봤지만 30일 이상 다시 보지 않은 콘텐츠를 viewed_at ASC 순으로 최대 10개 노출.
...
- **Forgotten**: `viewed_at IS NOT NULL` + `viewed_at < 30일 전`. 봤지만 오래 안 본 콘텐츠
