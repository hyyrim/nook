...
Profile 화면의 `계정 삭제` 액션은 1depth 설정 목록에서 직접 노출하지 않고, `계정 관리` 액션 시트 안으로 이동한다.

### Why

- 계정 삭제는 App Store 정책상 제공되어야 하지만, 일반 설정 목록에서 빨간색으로 크게 보이면 앱의 기본 경험보다 위험 액션이 먼저 눈에 띈다
- Nook의 디자인 시스템에서 Accent Red는 CTA나 rediscover dot처럼 제한적으로 사용하는 편이 좋다
- 새 주요 화면을 추가하지 않고 기존 `ActionSheet`를 재사용하면 MVP 화면 범위를 늘리지 않으면서도 한 단계 더 신중한 흐름을 만들 수 있다

### Impact

- Profile > 계정 섹션에는 `로그아웃`과 `계정 관리`만 표시한다
- `계정 관리`를 누르면 액션 시트에서 `계정 삭제`를 선택할 수 있다
- 실제 삭제 전 기존 Alert 확인은 유지한다
- `로그아웃`은 일반 계정 액션 색상으로 낮춰 빨간색 노출을 줄인다
